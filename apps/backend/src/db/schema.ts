...
export const triggerRun = pgTable(
  ...
  (t) => ({
    ...
    // `notification.read`); the loop breaker counts by this column and ignores
    // null, so those firings are exempt by construction.
  ...
  (t) => [
    ...
    // The loop breaker's per-(Trigger, entity) count over its rolling window,
