---
title: Dashboards
description: Build a dashboard in a Workspace — add widgets (metric, text, image, embeds, weather, and charts), arrange them in an editable grid, and let Agents keep widget data up to date.
---
...
discard them and exit edit mode. If **Save** fails partway, click it again: it
retries only what didn't go through. Widgets a failed **Save** already deleted
stay deleted if you then **Cancel**, and the dashboard tells you how many.
...
| **Embed**           | An HTTPS web page inside the dashboard                           |
...
<Callout type="warning">
  An Embed can appear blank for two reasons. Some sites refuse to be embedded at
  all — choose a site that explicitly supports embedding. Others embed fine but
  show nothing until you are signed in to them: an Embed is kept isolated from
  the rest of Platypus, so it never carries your session with it, and a page
  that needs its own sign-in stays empty even when the same page works in a
  normal browser tab. Prefer a URL that is shareable without a login, such as a
  Grafana snapshot or a published Sheet.
</Callout>
...
<Callout type="warning">
  ...
  Agent fills in the numbers. Embed URLs are human-owned and cannot be changed
  by Agents. (Updating another widget also requires matching its type, so an
  Agent can't accidentally turn a chart into a metric.)
</Callout>
