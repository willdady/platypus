...
- the Provider has no search of its own **and** no Web-search backend selected —
  which covers **Bedrock**, and **OpenAI** with **API Mode** set to **Chat
...
- an Org Admin has switched **Native web search** off on that Provider. That
  switch currently disables a selected backend too, not only the vendor's search;
...
[Tool sets](/building-with-platypus/tool-sets). Which search a Provider uses is
