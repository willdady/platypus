...
<Callout type="warning">
  **A Web-search backend can also read pages, not just search.** Where one is
  selected, turning **Search** on may hand the model a page reader for that turn
  as well — it can fetch a URL it found in the results, or one you pasted into
  the message. A vendor's built-in search never does this; it returns results
  and nothing else. Whether you get the reader is the backend's choice, not a
  separate switch, so the only control over it is which backend the Provider
  points at.
</Callout>
...
Search is a Provider setting, not a tool you grant. An Agent that should read
specific pages whenever it likes wants the **Web Fetch** Tool set — see
[Tool sets](/building-with-platypus/tool-sets) — which stays available every
turn, where a backend's page reader exists only on a turn you searched. Which search a Provider uses is
in [Providers & models](/concepts/providers#web-search).
