...
**Advanced settings** if the run needs your Memory summaries — see [Memories in a
run](#memories-in-a-run). Click save.
...
Off is the default because it keeps a firing repeatable. Summaries are written in
the background from the Chats you hold elsewhere, so a Trigger with them on
composes a different system prompt from one day to the next while its Instruction,
its Agent and its cadence have not changed. Turn them on where the Instruction is
about you — a daily briefing, a summary written in your voice — and leave them off
for a narrow, repetitive job.

<Callout type="warning">
  Upgrading to this version turns Memories **off for every Trigger you already
  have**. Before it, every run carried them, and nothing was backfilled to
  preserve that. A Trigger whose Instruction quietly relied on a summary — "the
  project I mentioned", "my usual format" — has nothing to read it from on its
  next firing, and the run does not fail, it just answers differently. Turn
  **Include Memories** back on for those Triggers, or write the fact into the
