...
- **Status** — all, or one of Success, Failed, Running, Pending.
...
A run that never happened leaves no row in **Trigger runs**, so the answer is in
the backend logs. Every time an event reaches a Trigger that subscribes to it,
Platypus writes one **dispatch decision** line naming the event, the Trigger, the
Trigger's Agent, the Agents that were running when the write happened, and one of
three outcomes:

- **fired** — the run is scheduled, and starts once the debounce window closes.
- **skipped_self_actor** — the Trigger's own Agent (or one of its Sub-Agents)
  made the write, so running it again would loop.
...
A Trigger whose event or filters didn't match writes no line at all — that, and
the absence of a decision, is how you tell a filter problem from a suppressed
run. The lines name identifiers only, never Card titles or body text.
