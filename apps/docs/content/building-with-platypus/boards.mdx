...
<Callout type="warning">
  The same holds when you change a card's **Column** in the card dialog. Your
  other edits to the card are saved, but the card stays where the Agent put it:
  you are told it was not moved, the dialog closes, and the board reloads. Open
  the card again to move it from where it now is.
</Callout>
