...
If you want to get a chat running, head to
...
An **Organization** is the top level — your company, your team, or you alone.
...
People who administer an Organization (adding members, configuring credentials)
are **Org Admins**.
...
Each Workspace is owned by exactly one person — the **Workspace Owner** — and has
no other members. Think of it as your personal project space: what you build in
one Workspace doesn't leak into anyone else's. It is not private from Org Admins,
though: they can open any Workspace in the Organization.

<Callout type="info">
  Members don't create Workspaces. An Org Admin creates one for you, usually
  with a Provider — and optionally a Sandbox — already in place. When you accept
  an invitation, a Workspace is provisioned for you automatically, fitted out by
  any [Blueprints](/concepts/sharing) on the invitation.
</Callout>
...
A Chat turn runs on the server, so it keeps going if you close the tab.
[Chat](/building-with-platypus/chat) covers that and the rest of the screen.
...
The [Agents & Sub-Agents](/concepts/agents) page goes deeper.
