...
- **contributed by a plugin** — a plugin declares tool sets in its manifest and
  ...
  Agent form's **Tools** card is the live list. The built-in ones are catalogued
  in [Tool sets](/building-with-platypus/tool-sets); [Extending → Tool
  sets](/extending/tool-sets) covers contributing your own.
