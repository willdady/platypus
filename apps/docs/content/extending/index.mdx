...
**Advanced settings → Web search** lists **None**, the Provider's own search
where it has one, and every backend the deployment has installed, by the
display name your manifest gives it. Selecting your backend takes precedence
over the Provider's built-in search, and the Chat **Search** toggle appears on
that Provider even when its model has no search of its own — which is the
case this Extension point exists for. The same field is on the Provider API as
`searchSource`.
