...
| Serve the Chat **Search** toggle from your own search service              | **[Web-search backend](/extending/web-search-backends)** contribution | Core owns the two tools and the toggle; you supply the calls to your upstream |
...
such as vLLM, llama.cpp or LiteLLM. You supply two executors, a mandatory
`web_search` and an optional `read_url`; core owns the tools built around them.
**[Web-search backends](/extending/web-search-backends)** has the contract, the
caps, and the egress guard.

An Org Admin points a Provider at your backend from the Provider form under
**Advanced settings → Web search**, by the display name your manifest gives it.
The same field is on the Provider API as `searchSource`.
