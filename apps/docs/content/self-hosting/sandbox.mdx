...
2. An opt-in compose overlay, `compose.sandbox.yaml`, which adds the socket mount
   and adds `@platypus/docker` to the `PLATYPUS_PLUGINS` list from your `.env`.
...
<Callout type="warning">
  **Don't also list `@platypus/docker` in `.env`.** The overlay adds it for you,
  so listing it yourself names it twice and boot aborts with `Plugin
  "@platypus/docker" is listed more than once in PLATYPUS_PLUGINS`. The rest of
  your `.env` list keeps loading — see
