...
**Trigger run**:
One execution of a **Trigger** — the headless shape of a **Drive**. Recorded separately from any Chat under its own status vocabulary (`pending` / `running` / `success` / `failed` / `suppressed` — not the chat-run words), with its own stats and its own retention: Max Runs to Keep bounds the history, everything inside the run-rate cap's window is kept so the cap's count stays countable, and _suppressed_ rows have a budget of their own. A _suppressed_ Trigger run is a firing the run-rate cap dropped before the Agent started, so it never ran; the row is the visible trace of the cap tripping. Bounded by the same **Output ceiling** and **Step ceiling** as any Drive, plus the unattended-only no-progress stop: repeating the same tool call and getting the same result several times in a row ends the run as _failed_, naming the tool — distinct from a step-limit stop, which still ends _success_.
